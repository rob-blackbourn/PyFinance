...
from core import amod
from py_calendrical.gregorian_date import JulianMonth
from py_calendrical import julian_date.JulianDate
...
class RomanDate(object):
    ...
    YEAR_ROME_FOUNDED = julian_date.bce(753)
    ...
    def ides_of_month(cls, month):
        ...
        return 15 if month in [CalendarMonth.March, CalendarMonth.May, CalendarMonth.July, CalendarMonth.October] else 13
    ...
    def to_fixed(self):
        ...
        return ({Event.Kalends: julian_date(self.year, self.month, 1).to_fixed(),
                 Event.Nones:   julian_date(self.year, self.month, self.nones_of_month(self.month)).to_fixed(),
                 Event.Ides:    julian_date(self.year, self.month, self.ides_of_month(self.month)).to_fixed()
                 }[self.event] -
                self.count +
                (0 if (julian_date.is_leap_year(self.year) and
                       (self.month == CalendarMonth.March) and
    ...
    def from_fixed(cls, fixed_date):
        ...
        julian_date = julian_date.from_fixed(fixed_date)
        ...
        if julian_date.day == 1:
            ...
            return RomanDate(julian_date.year,
                             ...
                             False)
        elif (julian_date.month != CalendarMonth.February) or not julian_date.is_leap_year(julian_date.year):
            ...
            return RomanDate(julian_date.year, CalendarMonth.March, Event.Kalends, 30 - julian_date.day, False)
        else:
            return RomanDate(julian_date.year, CalendarMonth.March, Event.Kalends, 31 - julian_date.day, julian_date.day == 25)
