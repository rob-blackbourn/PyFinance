...
def urbana_winter(g_year):
    ...
    return URBANA.standard_from_universal(Solar.solar_longitude_after(Astro.WINTER, GregorianDate(g_year, JulianMonth.January, 1).to_fixed()))
...
def asr(date, location):
    ...
    delta = Astro.declination(noon, 0, Solar.solar_longitude(noon))
...
def astronomical_easter(g_year):
    ...
    paschal_moon = ifloor(JERUSALEM.apparent_from_local(JERUSALEM.local_from_universal(Lunar.lunar_phase_at_or_after(Lunar.FULL, equinox))))
