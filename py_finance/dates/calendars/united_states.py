...
class UnitedStatesNyse(YearlyCalendar):
    ...
    def __generatte_holidays(cls, year):
        ...
        new_years_day = date(year, CalendarMonth.january, 1)
        yield (new_years_day if new_years_day.weekday() != DayOfWeek.sunday else new_years_day + timedelta(1))
        
        # Washington's birthday (third Monday in February)
        yield empty.add_nth_day_of_week(date(year, CalendarMonth.february, 1), 3, DayOfWeek.monday, False)
        ...
        if year <= 1968 or (year <= 1980 and year % 4 == 0):
            ...
            yield Calendar.add_nth_day_of_week(date(year, CalendarMonth.november, 1), 1, DayOfWeek.tuesday, True)

        # Special closings
        if year == 2012:
            # Hurricane Sandy
            yield date(2012, CalendarMonth.october, 29)
            yield date(2012, CalendarMonth.october, 30)

        if year == 2007:
            # President Ford's funeral
            yield date(2007, CalendarMonth.january, 2)
        
        if year == 2004:
            # President Reagan's funeral
            yield date(2004, CalendarMonth.june, 11)
        
        if year == 2001:
            # September 11-14, 2001
            yield date(2001, CalendarMonth.september, 11)
            yield date(2001, CalendarMonth.september, 12)
            yield date(2001, CalendarMonth.september, 13)
            yield date(2001, CalendarMonth.september, 14)
            
        if year == 1994:
            # President Nixon's funeral
            yield date(1994, CalendarMonth.april, 27)
            
        if year == 1985:
            # Hurricane Gloria
            yield date(1985, CalendarMonth.september, 27)
            
        if year == 1977:
            # 1977 Blackout
            yield date(1977, CalendarMonth.july, 14)
            
        if year == 1973:
            # Funeral of former President Lyndon B. Johnson.
            yield date(1973, CalendarMonth.january, 25)
            
        if year == 1972:
            # Funeral of former President Harry S. Truman
            yield date(1972, CalendarMonth.december, 28)
            
        if year == 1969:
            # National Day of Participation for the lunar exploration.
            yield date(1969, CalendarMonth.july, 21)
            # Funeral of former President Eisenhower.
            yield date(1969, CalendarMonth.march, 31)
            # Closed all day - heavy snow.
            yield date(1969, CalendarMonth.february, 10)
            
        if year == 1968:
            # Day after Independence Day.
            yield date(1968, CalendarMonth.july, 5)
            # June 12-Dec. 31, 1968
            # Four day week (closed on Wednesdays) - Paperwork Crisis
            d = date(1968, CalendarMonth.june, 12)
            while d.year == 1968:
                yield d
                d += timedelta(7)
            # Day of mourning for Martin Luther King Jr.
            yield date(1968, CalendarMonth.april, 9)
        
        if year == 1963:
            # Funeral of President Kennedy
            yield date(1963, CalendarMonth.november, 25)
        
        if year == 1961:
            # Day before Decoration Day
            yield date(1961, CalendarMonth.may, 29)
            
        if year == 1958:
            # Day after Christmas
            yield date(1958, CalendarMonth.december, 26)
            
        if year == 1954 or year == 1956 or year == 1965:
            # Christmas Eve
            yield date(year, CalendarMonth.december, 24)
